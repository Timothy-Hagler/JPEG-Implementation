function dequantizedImage= Dequantize(image, N, qf)
    disp("running dequant");
    if qf > 100
        qf = 100;
    elseif qf < 1
        qf = 1;
   ...
   for i = 1:N:rowsize
        for j = 1:N:colsize
            if i+N-1 <= rowsize && j+N-1 <= colsize
                ...
                dequantizedImage(i:i+N-1,j:j+N-1,1) = newY;
                dequantizedImage(i:i+N-1,j:j+N-1,2) = newCb;
                dequantizedImage(i:i+N-1,j:j+N-1,3) = newCr;

             end
        end
    end
    dequantizedImage = uint8((dequantizedImage));
end